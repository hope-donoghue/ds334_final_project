...
```{r}
table(shots_valid_wwc2023_test_1$shot.type.name, shots_valid_wwc2023_test_1$is.goal)
```
...
# New from 4/29
# USE THIS
# filter out backheel, diving header, overhead kicks, from kick off in play_pattern.name
# as there were 0 goals there

# filter out shot.type.name Free Kick as there is only 1 goal off a free kick along with 41 missed attempts
# saw that this is probably confounded so drop shot.type.name entirely

shots_valid_wwc2023_new <- shots_valid_wwc2023_test_1 %>%
  filter(shot.technique.name != "Backheel") %>%
  filter(shot.technique.name != "Diving Header") %>%
  filter(shot.technique.name != "Overhead Kick") %>%
...
shots_valid_wwc2023_new <- shots_valid_wwc2023_new %>%
  filter(distance.to.gk != "NA")
```


```{r}
# Logistic regression 
# see that shot.type.namePenalty has a very large standard error

library(broom)


goal_glm <- glm(as.factor(is.goal) ~ distance.to.gk + DistToGoal + angle.to.gk + angle.to.goal + gk.to.goalline + play_pattern.name + shot.body_part.name + shot.type.name + shot.technique.name, data = shots_valid_wwc2023_new, family = "binomial")
...
goal_glm <- glm(as.factor(is.goal) ~ DistToGoal + play_pattern.name + shot.body_part.name + shot.technique.name, data = shots_valid_wwc2023_new, family = "binomial")
...
```{r}
library(modelr)
grid <- shots_valid_wwc2023_new %>%
  data_grid(
    DistToGoal = seq_range(DistToGoal, n = 5),
    play_pattern.name = c("From Counter", "From Goal Kick", "From Keeper", "From Throw In", "Other", "Regular Play"),
    shot.body_part.name = c("Head", "Left Foot", "Other", "Right Foot"),
    shot.technique.name = c("Half Volley", "Lob", "Normal", "Volley"))
...
```{r}
rug_goal <- shots_valid_wwc2023_new %>%
  filter(goal == 1)

rug_nogoal <- shots_valid_wwc2023_new %>%
  filter(goal == 0)

ggplot(data = aug_prob, aes(x = shot.technique.name, y = .fitted)) + geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  theme_minimal() + 
  geom_rug(data = rug_goal, sides = "t", alpha = 0.3, aes(y = goal)) +
  geom_rug(data = rug_nogoal, sides = "b", alpha = 0.3, aes(y = goal))  

```

```{r}
# goals scored on top, goals not scored on bottom 
ggplot(data = aug_prob, aes(x = DistToGoal, y = .fitted)) + geom_line(data = aug_prob, aes(color = play_pattern.name), linewidth = 2)  + facet_wrap(~shot.technique.name)+ geom_rug(data = rug_goal, sides = "t", alpha = 0.3, aes(y = goal)) + geom_rug(data = rug_nogoal, sides = "b", alpha = 0.3, aes(y = goal))
```
