...
rf.1$bestTune$mtry
```
- The selected model uses 6 randomly selected predictors for the highest accuracy
...
```
- We see that the accuracy is 91.65%
...
```{r}
shots_valid_wwc2023_no_corner <- left_join(shots_valid_wwc2023_no_corner, shots.varsdata[,c("id", "my.xG")], by = 
                               "id")
```


```{r}
# shot map showing my model's xG for the 2023 WWC without Ireland's goal from corner
...
```{r}
# shot map with goals and xG size of point without Ireland's goal from corner
a4 = pitch_2 + geom_point(data = shots_valid_wwc2023_no_corner, aes(x = location.x, y = location.y, size = my.xG$Goal, color = shot.outcome.name == "Goal")) + labs(size = "xG", color = "Goal", caption = "Removed Ireland's goal directly from corner")
