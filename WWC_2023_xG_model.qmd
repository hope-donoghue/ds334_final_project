...
```
- We see that the accuracy is 91.89%
...
```
We see that DistToGoal and distance.to.gk are important features in the model (100% and 89.44% respectively)
...
players_data <- shots_valid_wwc2023_test_1 %>%
  filter(player.name %in% c("Jill Roord"))

plotly_shot_map <- ggplot(data = players_data, aes(x = location.x, y = location.y, color = my.xG$Goal, stroke = is.goal_numeric, label = opponent)) + annotate_pitch(dimensions = pitch_statsbomb) + theme_pitch() + coord_flip(xlim = c(55, 120), ylim = c(-12, 105)) + geom_point(shape = 1, size = 1.5)


ggplotly(plotly_shot_map, tooltip = c("color", "label"))
```
