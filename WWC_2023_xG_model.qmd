...
```{r}
table(shots_valid_wwc2023_new$play_pattern.name, shots_valid_wwc2023_new$is.goal)
```
...
goal_glm <- glm(as.factor(is.goal) ~ DistToGoal + AngleToGoal + DistToKeeper + AngleToKeeper + play_pattern.name + shot.body_part.name + shot.technique.name, data = shots_valid_wwc2023_new, family = "binomial")
