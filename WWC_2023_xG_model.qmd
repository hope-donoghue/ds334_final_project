...
```{r}
table(shots_valid_wwc2023_test_1$play_pattern.name, shots_valid_wwc2023_test_1$is.goal)
```
...
ggplot(data = aug_prob, aes(x = DistToGoal, y = .fitted)) + geom_line(data = aug_prob, aes(color = play_pattern.name), linewidth = 2)  + facet_wrap(~shot.type.name)+ geom_rug(data = rug_goal, sides = "t", alpha = 0.3, aes(y = goal)) + geom_rug(data = rug_nogoal, sides = "b", alpha = 0.3, aes(y = goal))
```

Question on what the plot should look like for logistic regression...
Plot looks very confusing with lots of lines
